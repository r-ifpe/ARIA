...
rename_comparison_data_frame <- function(x) {
  ...
  rfept_matricula_equivalent <- rfept_rename_column(rfept_table, "MATRICULA_EQUIVALENTE")
  ...
  x %>%
    dplyr::select(
      ...
      !!rfept_matricula_equivalent := !!sym("R_NO_STATUS_MATRICULA_SISTEC"),
...
rename_wrong_beginning_data_frame <- function(x) {
  ...
  x %>%
    dplyr::select(
      ...
      CAMPUS = !!sym("R_NO_CAMPUS"),
      CURSO = !!sym("R_NO_CURSO")
    ) # to use rfept is better to update
}
