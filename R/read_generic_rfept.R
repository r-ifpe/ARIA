...
read_generic_rfept <- function(path = "", start = NULL) {
  ...
  sep <- detect_sep(temp)
  encoding <- detect_encoding(temp, sep, "NO_CURSO")

  rfept <- utils::read.csv(
    temp,
...
check_rfept_header <- function(x) {
  ...
  columns <- c(
    stringr::str_detect(header, "NO_ALUNO"),
  ...
  if (sum(columns) != 7) {
    rfept_header <- c(
      "NO_ALUNO", "NU_CPF", "CO_MATRICULA", "NO_STATUS_MATRICULA",
...
check_rfept_status <- function(x) {
  all_true <- all(stringr::str_detect(
    x$R_NO_STATUS_MATRICULA,
